//#define I2S_WS_TX  15
//#define I2S_SCK_TX 2
//#define I2S_DATA_OUT_TX  13
//#define I2S_PORT_TX I2S_NUM_0
//
//
//#define I2S_WS_RX  26
//#define I2S_SCK_RX 27
//#define I2S_SD_RX  25
//#define I2S_PORT_RX I2S_NUM_1


#define I2S_WS_TX  26 // KE SPEAKER
#define I2S_SCK_TX 27
...
void i2s_adc_data_scale(uint8_t* d_buff, uint8_t* s_buff, uint32_t len, float volume) {
    uint32_t j = 0;
    uint32_t dac_value = 0;
    ...
    for (int i = 0; i < len; i += 2) {
        ...
        dac_value = (uint32_t)(dac_value * volume);
...
void i2s_read_data() {
  ...
  i2s_adc_data_scale(flash_write_buff, (uint8_t*)i2s_read_buff, I2S_READ_LEN,0.5);
}
...
void i2s_write_data(char *buf_ptr, int buf_size, float volume) {
  int samples = buf_size / 4; // Jumlah sampel dalam buffer (asumsikan format int32_t)

  // Mengalikan amplitudo data audio dengan faktor gain
  for (int i = 0; i < samples; i++) {
    int32_t *sample_ptr = (int32_t*)&buf_ptr[i * 4]; // Pointer ke sampel audio

    // Mengalikan nilai sampel dengan faktor gain
    *sample_ptr = (int32_t)(*sample_ptr * volume);
  }
